...
const setCurrentLanguage = (id) => {
  ...
  header.insertAdjacentHTML(
    "beforeend",
    `<h1 class="targetLanguage">${getLanguageName(id)}</h1>`
  );
};
...
>>>>>>> dd0bd0f1a3d31979ea43e1a1a2fd4388ab4c8dfe
...
=======
  let translation = "ice cream";
  let newVocabInputs = document.getElementById("newVocabInputs");
  newVocabInputs.insertAdjacentHTML(
    "beforeend",
    `<div class="targetLanguage"><span>${translation}</span></div>`
  );
  // addWordToDBs(string);
  if (inputCount < 10) {
    newVocabInputs.appendChild(createInput());
...
const startScreen = () => {
  ...
  otherLanguagesIds = allLanguagesIds.filter(
    (a) => !userWordsArray.includes(a)
  );
  root.innerHTML = "";
  root.classList.toggle("startScreen");
...
let getSessionWords = () => {
  sessionWords = userWords.filter(
    (a) => a.user_id == thisUser && a.language_id == currentLanguage
  );
};
...
const displayResult = (value) => {
  ...
  if (
    (value && sessionWords.length > 0) ||
    (!value && sessionWords.length > 1)
  ) {
...
const checkAnswer = (string) => {
  ...
  if (string == currentCard.translation) {
    ...
    let sessionWordsIndex = sessionWords.findIndex(
      (a) => a.word_id == currentCard.word_id
    );
